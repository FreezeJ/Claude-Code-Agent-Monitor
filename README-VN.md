...
| **Ứng dụng máy tính để bàn macOS** | Một workspace `desktop/` tùy chọn đóng gói toàn bộ bảng điều khiển thành ứng dụng `.app` macOS gốc (phân phối dưới dạng `.dmg`), được xây dựng bằng Electron 35. Đây là vỏ Electron **nhúng máy chủ Express hiện có ngay trong tiến trình** (`require()` trực tiếp `server/index.js` — không có tiến trình con, không có IPC) và hiển thị ứng dụng React đã build trong một `BrowserWindow`. Bổ sung vòng đời macOS gốc: biểu tượng menu-bar (tray), trình đơn ứng dụng gốc, tự khởi động lúc đăng nhập qua macOS Login Items / `SMAppService`, đóng cửa sổ thì ẩn đi nhưng máy chủ vẫn chạy, khóa một-phiên-bản, cùng các lệnh "Open in Browser", "Restart Server" và "Show Logs". Ưu tiên cổng 4820 (lùi về 4821–4829 rồi cổng cao ngẫu nhiên) và sẽ tiếp quản một máy chủ khỏe mạnh sẵn có trên 4820 thay vì bind trùng. Lần khởi động đầu tiên với máy chủ do ứng dụng sở hữu sẽ tự cài hook Claude Code và chạy các dịch vụ nền. Xem [Ứng dụng máy tính để bàn macOS](#ứng-dụng-máy-tính-để-bàn-macos) |
...
Nếu bạn không muốn giữ một cửa sổ terminal mở, hãy cài đặt **ứng dụng máy tính để bàn Electron** tùy chọn. Nó nhúng máy chủ ngay trong tiến trình, thêm biểu tượng menu-bar (tray) và hỗ trợ tự khởi động lúc đăng nhập qua Login Items gốc của macOS.
...
| `npm run desktop:dmg`   | Tạo DMG **universal** (x64 + arm64) — đúng cho bản phát hành. **Chậm.** |
| `npm run desktop:dmg:arm64` | Tạo DMG chỉ cho Apple Silicon. **Nhanh.** |
| `npm run desktop:dmg:x64`   | Tạo DMG chỉ cho Intel. **Nhanh.** |
...
Bảng điều khiển đi kèm một **ứng dụng macOS gốc** tùy chọn, đóng gói máy chủ và máy khách hiện có thành một tệp `.app` duy nhất mà bạn cài đặt một lần rồi quên đi. Mọi thứ bạn thấy trong trình duyệt tại `localhost:4820` đều nằm bên trong cửa sổ này, cộng thêm vòng đời gốc của macOS: biểu tượng menu-bar, trình đơn ứng dụng, tích hợp Login Items và một nút thoát duy nhất dọn dẹp máy chủ gọn gàng.

<p align="center">
  <img src="images/macos.png" alt="Claude Code Monitor chạy như một ứng dụng macOS gốc" width="100%">
  <br>
  <em>🍎 <strong>Ứng dụng macOS gốc</strong> — vỏ <code>.app</code> với biểu tượng menu-bar (tray), Open-at-Login và khoá single-instance. Cùng một bảng điều khiển, chạy trong một cửa sổ macOS thật.</em>
</p>
...
> [!LƯU Ý]
> **Trạng thái:** v1, chỉ dành cho macOS. Bản dựng Windows / Linux và trình tự cập nhật (auto-updater) nằm ngoài phạm vi của phiên bản này và được theo dõi như các việc tiếp nối.
...
| Tự khởi động lúc đăng nhập (macOS) | ❌ | ✅ qua Login Items gốc |
| Biểu tượng menu-bar (tray) cho trạng thái luôn-bật | ❌ | ✅ |
...
> [!QUAN TRỌNG]
> **Bản dựng universal cố tình chậm — hãy dùng bản dựng theo kiến trúc.** Lệnh `npm run desktop:dmg` tạo một DMG **universal** (build hai lần, hợp nhất cả hai kiến trúc x64 + arm64 và ký từng tệp nhị phân) nên mất khá lâu. Khi xây dựng cho **chính máy Mac của bạn**, hãy dùng `npm run desktop:dmg:arm64` (Apple Silicon) hoặc `npm run desktop:dmg:x64` (Intel) — chỉ một kiến trúc, không hợp nhất, hoàn tất trong khoảng một phút. CI đã dựng sẵn DMG universal và tải lên dưới dạng artifact `ClaudeCodeMonitor-dmg`, nên bạn hiếm khi cần tự dựng bản universal. DMG khoảng ~80 MB, ~250 MB khi giải nén trên đĩa.
...
5. Cửa sổ bảng điều khiển mở ra (trừ khi macOS khởi chạy ứng dụng lúc đăng nhập — khi đó nó ở chế độ chỉ-tray).

### Biểu tượng tray, trình đơn và tự khởi động

- **Tray (menu-bar)** — bề mặt luôn-bật. Trình đơn ngữ cảnh gồm: *Open Dashboard, Open in Browser, Restart Server, Show Logs, Open at Login (chuyển đổi), Quit*. Trình đơn được dựng lại mỗi lần mở nên nhãn cổng và ô đánh dấu *Open at Login* luôn cập nhật.
...
| `npm run desktop:dmg` | DMG **universal** (x64 + arm64). Đúng cho bản phát hành. **Chậm.** |
| `npm run desktop:dmg:arm64` | DMG chỉ cho Apple Silicon. **Nhanh (~1 phút).** |
| `npm run desktop:dmg:x64` | DMG chỉ cho Intel. **Nhanh (~1 phút).** |
...
DMG được **ký ad-hoc theo mặc định** (`CSC_IDENTITY_AUTO_DISCOVERY=false`) để ai cũng dựng được một `.dmg` hoạt động mà không cần tài khoản Apple Developer trả phí.
...
Một DMG ký ad-hoc sẽ kích hoạt cảnh báo Gatekeeper trong lần mở đầu tiên (*"Apple could not verify…"*). Cách giải quyết một dòng:
...
Job `🍎 macOS Desktop (DMG)` trong `.github/workflows/ci.yml` chạy trên `macos-latest`, được **lọc theo đường dẫn** (một job `changes` dùng `dorny/paths-filter` phát hiện thay đổi trong `desktop/**`). Nó cài phụ thuộc, build TypeScript, chạy kiểm thử smoke, dựng DMG universal rồi tải lên dưới dạng artifact **`ClaudeCodeMonitor-dmg`** tải xuống được từ lần chạy workflow.
...
```mermaid
flowchart LR
    subgraph desktopmode["Desktop — 1 process (Electron Main)"]
